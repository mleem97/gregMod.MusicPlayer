...
// Panel chrome behind an interface: with gregCore (GregPanel: slide/drag/
// registry) or standalone (own UIDocument, immediately visible, drag).
// Callers may ONLY touch the interface (JIT safety without DLL).
...
public static class PanelChromeFactory
{
    ...
    // --- gregCore implementation (only call when the DLL is present!) ---
    private sealed class CoreChrome : IPanelChrome
    {
        ...
        public void Tick() { /* Slide/drag is driven by the central registry */ }
    }
    ...
    private sealed class LocalChrome : IPanelChrome
    {
        ...
        public void Rebuild(Action<VisualElement> build)
        {
            ...
            {
                MelonLogger.Warning("[MusicPlayer] Panel rebuild failed: " + ex.Message);
            }
        }
